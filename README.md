...
### 3. Backend class
The Backend class is pretty much self-explaining. For our example, we expose *userName* and *items* properties.  The only thing to mention in regard of read/write properies is mandatory NOTIFY member in Q_PROPERTY definition. Public slots and signals exposed as JavaScript object's methods and events.
```c++
